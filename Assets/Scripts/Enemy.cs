...
public class Enemy : GameBehaviour<ArchersGameRules>
{
    ...
    [DoNotSerialize, NonSerialized] public Transform _Character, _Fance, _Base;
    ...
    {
        var obj = _Fance ?? _Character ?? _tower?.transform;
        if (obj != null)
        {
            var abilitySystem = GameplayAbilitySystem.FindAbilitySystem(obj.gameObject);
            if (abilitySystem != null && damageEffect != null)
            {
                abilitySystem.ApplyGameplayEffectToTarget(damageEffect.EffectType, obj.gameObject,
            ...
            if (!isDead)
            {
                if (_Fance == null)
                {
                    ...
                    if (Physics.Linecast(
                            ...
                            transform.position + transform.forward * 1f, out hit, fanceLayerMask))
                    {
                        _Fance = hit.transform;
                    }

                    if (Physics.Linecast(
                            new Vector3(transform.position.x - 0.25f, transform.position.y + 0.5f,
                                transform.position.z), transform.position + transform.forward * 1f, out hit,
                            fanceLayerMask))
                    {
                        _Fance = hit.transform;
                    }
        ...
        if (_stateAnim != stateE)
        {
            ...
            if (stateE == AnimationStateE.attak)
            {
                if (attak != null)
                {
                    ...
                    eventState.Events.Add(attackTimeEvent, ApplyDamage);
                }
                else
                {
                    if (_AnimancerComponent != null) _AnimancerComponent.Stop();
